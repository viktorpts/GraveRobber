...
public class Tile implements IRenderable {
    ...
    public Image setImageTile(TileType tileType) {
        if (tileType == TileType.WALL) {
            ...
            img = new Image("door.jpg");
        } else if (tileType == TileType.OPEN) {
            img = new Image("open.jpg");
        } else
    ...
    public void toggle() {
        if (tileType == TileType.DOOR) {
            ...
            img = setImageTile(TileType.OPEN);
        }
    }

    public void render() {
        QuickView.renderBlock(this.img, x - 1, y - 1, tileType, transparent);
    }
}
